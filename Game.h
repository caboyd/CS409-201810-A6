...
inline void Game::init()
{
  ...
  inline void Game::update(double fixed_delta_time)
{
...
inline void Game::updateAnimations(double delta_time)
{
 ...
 inline void Game::display()
{
	...
	count++;
	if (count % unsigned(FPS_DISPLAY / 10) == 0)
	{
		display_fps = int(1000.0 / delta_time);
		update_fps = int(update_count / (elapsed_time_milliseconds/1000.0));
	}

	text_renderer.draw("Update Rate: " + std::to_string(update_fps), 2, float(win_height - 24), 0.4f, glm::vec3(0, 1, 0));
	text_renderer.draw("Display Rate: " + std::to_string(display_fps), 2, float(win_height - 44), 0.4f, glm::vec3(0, 1, 0));
...
inline void Game::destroyIntoNextWorld()
{
	world.destroy();
...
inline void Game::renderToDepthTexture(glm::mat4& depth_vp)
{
